...
describe('Submit passes', () => {
  ...
  it('submit correctly', async () => {
    ...
    //Check that the admin is able to see the radio buttons with the label admin and user
    expect(getByTestId('screen')).toHaveTextContent('Admin');
    expect(getByTestId('screen')).toHaveTextContent('User');
    ...
    userEvent.type(confirmPassword, 'hoi');
    userEvent.type(dateOfBirth, '1994-02-01');
    userEvent.click(getByTestId('admin'));

    userEvent.click(getByTestId('submit-button'));
    await waitFor(() => expect(window.location.replace).toHaveBeenCalledWith('/Admin/Catalog'));
  });
});
...
describe('Submit fails', () => {
  ...
  it('displays message when there is a 400 error', async () => {
    ...
    const failPassword = await findByTestId('userFail-message');

    expect(failPassword).toBeTruthy();
  });
  ...
  it('Shows error too young', async () => {
    ...
    let screen = await findByTestId('screen');
    expect(screen).toHaveTextContent('Sorry je bent te jong. Je moet minstens 18 zijn om binnen te komen');
  });
});
