...
const { userOne, admin, insertUsers, genders, uni } = require('../fixtures/user.fixture');
...
describe('Auth routes', () => {
  ...
  describe('POST /api/auth/reset-password', () => {
    ...
    test('should return 204 and reset the password and send confirmation email to the user', async () => {
      await insertUsers([userOne]);
      ...
      const confirmPasswordResetEmailSpy = jest.spyOn(emailService, 'confirmPasswordResetEmail');
      ...
      expect(confirmPasswordResetEmailSpy).toHaveBeenCalledWith(userOne.email);
    });
...
describe('Auth middleware', () => {
  ...
  test('should call next with no errors if not rightsOnly and user does not have required rights but userId is in params', async () => {
    await insertUsers([userOne]);
    const req = httpMocks.createRequest({
  ...
  test('should call next with forbidden if rightsOnly and user does not have required rights but userId is in params', async () => {
    await insertUsers([userOne]);
    const req = httpMocks.createRequest({
      headers: { Authorization: `Bearer ${userOneAccessToken}` },
      params: { userId: userOne._id.toHexString() },
    ...
    await auth(true, 'anyRight')(req, httpMocks.createResponse(), next);
  ...
  test('should call next with no errors if user has required rights', async () => {
    ...
    await auth(false, ...roleRights.get('admin'))(req, httpMocks.createResponse(), next);
